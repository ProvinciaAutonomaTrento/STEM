...
#            com = las.union(items, out, compres, local= True) #self.LocalCheck.isChecked())
 #           STEMUtils.saveCommand(com)
            ...
            ###################### R script here ##############################
            processing.run("r:Unione_las", { 'FileLas' : source1, 'FileLas_2' :  source2, 'Output' : out})
            ###################################################################
...
#                las._pyroRelease()
            ...
            t = time.time()
            while not os.path.isfile(out):#out_locale
                if time.time()-t > 5:
                    STEMMessageHandler.error("{ou} LAS file not created".format(ou=out))#out_locale
                    return
                time.sleep(.1)
            STEMMessageHandler.success("{ou} LAS file created".format(ou=out))#out_locale
        except:
#            if not self.LocalCheck.isChecked():
#                las._pyroRelease()
