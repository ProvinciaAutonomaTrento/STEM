...
class STEMToolsDialog(BaseDialog):
    def __init__(self, iface, name):
        ...
        items = ['6s']
        label = "Seleziona l'algoritmo da utilizzare"
        self._insertFirstCombobox(label, 3, items)
...
#            local = self.LocalCheck.isChecked()
            ...
            if cut:
                name = cut
                source = cutsource
            tempin, tempout, gs = temporaryFilesGRASS(name, False)#local)
...
#################################################### 
            ...
            STEMUtils.exportGRASS(gs, self.overwrite, output, tempout, typ, local = True) #local)
