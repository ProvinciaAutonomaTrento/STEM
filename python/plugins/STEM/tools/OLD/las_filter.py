...
class STEMToolsDialog(BaseDialog):
    ...
    def __init__(self, iface, name):
        ...
        #filters = "LAS files (*.las);;LAZ files (*.laz);;All files (*.*)"
...
#            STEMUtils.saveCommand(com)
            ...
            ###################### R script here ##############################
            processing.run("r:Filtro_las", { 'FileLas' : source,'Output' : out, 'Seleziona_ritorno' : ret,
...
#                las._pyroRelease()
            t = time.time()
            while not os.path.isfile(out):  # out_orig
                if time.time()-t > 5:
                    STEMMessageHandler.error("{ou} file not created".format(ou=out)) # out_orig
                    return
                time.sleep(.1)
            STEMMessageHandler.success("{ou} file created".format(ou=out)) # out_orig
        except:
#            if not self.LocalCheck.isChecked():
#                las._pyroRelease()
