...
##Numero_di_alberi=optional number
...
if(is.null(Numero_di_alberi) == FALSE){
  supClass_rf <- superClass(immagine, trainData = Training_aree, responseCol = Seleziona_colonna_codice_classe, filename = Output_mappa, 
                             valData = Aree_validazione, model = "rf", overwrite = TRUE, mode = "classification", predict = Creazione_mappa, predType = "raw",
                             ntree = Numero_di_alberi, kfold = Numero_fold_cross_validation, verbose = TRUE, polygonBasedCV = TRUE)
  model <- supClass_rf$model
  if(is.null(Aree_validazione)==FALSE){
  accuracy <- supClass_rf$validation$performance
  }
} else {
  supClass_rf <- superClass(immagine, trainData = Training_aree, responseCol = Seleziona_colonna_codice_classe, filename = Output_mappa, 
                             valData = Aree_validazione, model = "rf", overwrite = TRUE, mode = "classification", predict = Creazione_mappa,
