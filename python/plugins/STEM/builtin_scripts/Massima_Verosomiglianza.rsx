...
if(is.null(Numero_fold_cross_validation) == FALSE){
  supClass_mlc <- superClass(immagine, trainData = Training_aree, responseCol = Seleziona_colonna_codice_classe, filename = Output_mappa, 
                            valData = Aree_validazione, model = "mlc", overwrite = TRUE, mode = "classification", kfold = Numero_fold_cross_validation, 
                            predict = Creazione_mappa, predType = "raw", verbose = TRUE, polygonBasedCV = TRUE)
  model <- supClass_mlc$model
  if(is.null(Aree_validazione)==FALSE){
  accuracy <- supClass_mlc$validation$performance
  }
} else {
  supClass_mlc <- superClass(immagine, trainData = Training_aree, responseCol = Seleziona_colonna_codice_classe, filename = Output_mappa, 
                             valData = Aree_validazione, model = "mlc", overwrite = TRUE, mode = "classification", predict = Creazione_mappa, 
