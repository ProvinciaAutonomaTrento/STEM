...
LINEAR = [dict(name='LinearRegression',
               classifier=LinearRegression, kwargs={})]

RIDGE = [dict(name='Ridge_a%f' % a, classifier=Ridge, kwargs={'alpha': a})
         for a in ALPHAS]

LASSO = [dict(name='Lasso_a%f' % a, classifier=Lasso, kwargs={'alpha': a})
         for a in ALPHAS]

ELAST = [dict(name='ElasticNet_a%f_r%f' % (a, r), classifier=ElasticNet,
...
LARS = [dict(name='Lars_c%s' % str(c), classifier=Lars,
             kwargs={'n_nonzero_coefs': c})
         for c in (1, 2, 3, 4, 5, 10, 25, 50, 100, np.inf)]

LLARS = [dict(name='LassoLars_a%f' % a, classifier=LassoLars,
              kwargs={'alpha': a})
         for a in ALPHAS]
...
PAR = [dict(name='PassiveAggressiveRegressor_c%f_l%s' % (c, l),
            classifier=PassiveAggressiveRegressor, kwargs={'C': c, 'loss': l})
...
SVRLIN = [dict(name='SVR_k%s_C%f' % (k, c), classifier=SVR,
               kwargs={'C': c, 'kernel': k, 'probability': True})
          for c in ALPHAS for k in ('linear', )]

SVRSIGRBF = [dict(name='SVR_k%s_C%f_g%f' % (k, c, g), classifier=SVR,
